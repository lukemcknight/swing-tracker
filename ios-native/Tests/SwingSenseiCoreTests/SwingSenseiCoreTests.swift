...
final class SwingSenseiCoreTests: XCTestCase {
    func testDecodesSwingAnalysisContract() throws {
        ...
        XCTAssertEqual(analysis.shotEstimate?.landingZone, "right")
        XCTAssertEqual(analysis.shotEstimate?.clubSpeedMphRange?.unit, "mph")
    ...
    func testDecodesSwingAnalysisWithoutShotEstimate() throws {
        let analysis = try makeFixtureAnalysis(includeShotEstimate: false)

        XCTAssertNil(analysis.shotEstimate)
        XCTAssertEqual(analysis.senseiScore, 8.4)
    }
    ...
    func testArcadeShotEngineUsesBallSpeedEstimate() {
        let hole = HoleState(definition: ArcadeCourseLibrary.starterCourse.holes[1])
        let estimate = makeShotEstimate(
            ballSpeed: SpeedRange(min: 106, max: 112),
            landingZone: "center",
            confidence: "high"
        )

        let result = ArcadeShotEngine().resolveShot(
            hole: hole,
            club: "7",
            swingID: "swing-arcade",
            shotEstimate: estimate,
            createdAt: Date(timeIntervalSince1970: 1),
            shotID: "shot-1"
        )

        XCTAssertEqual(result.shot.confidence, "high")
        XCTAssertEqual(result.shot.lie, .fairway)
        XCTAssertGreaterThan(result.shot.carryYards, 150)
        XCTAssertLessThan(result.shot.carryYards, 180)
        XCTAssertEqual(result.updatedHole.strokes, 1)
        XCTAssertFalse(result.updatedHole.completed)
    }

    func testArcadeShotEngineFallsBackToClubSpeedThenDefaultCarry() {
        let hole = HoleState(definition: ArcadeCourseLibrary.starterCourse.holes[1])
        let clubSpeedOnly = makeShotEstimate(
            clubSpeed: SpeedRange(min: 78, max: 82),
            landingZone: "center",
            confidence: "high"
        )

        let clubSpeedResult = ArcadeShotEngine().resolveShot(
            hole: hole,
            club: "7",
            swingID: "swing-club-speed",
            shotEstimate: clubSpeedOnly
        )
        let missingEstimateResult = ArcadeShotEngine().resolveShot(
            hole: hole,
            club: "7",
            swingID: "swing-missing",
            shotEstimate: nil
        )

        XCTAssertEqual(clubSpeedResult.shot.confidence, "high")
        XCTAssertGreaterThan(clubSpeedResult.shot.carryYards, 145)
        XCTAssertEqual(missingEstimateResult.shot.confidence, "low")
        XCTAssertEqual(missingEstimateResult.shot.carryYards, GolfClub.sevenIron.arcadeDefaultCarryYards * 0.86, accuracy: 0.001)
    }

    func testArcadeShotEngineWidensDispersionForLowConfidence() {
        let hole = HoleState(definition: ArcadeCourseLibrary.starterCourse.holes[1])
        let high = makeShotEstimate(
            ballSpeed: SpeedRange(min: 106, max: 112),
            landingZone: "center",
            confidence: "high"
        )
        let low = makeShotEstimate(
            ballSpeed: SpeedRange(min: 106, max: 112),
            landingZone: "center",
            confidence: "low"
        )

        let highResult = ArcadeShotEngine().resolveShot(hole: hole, club: "7", swingID: "high", shotEstimate: high)
        let lowResult = ArcadeShotEngine().resolveShot(hole: hole, club: "7", swingID: "low", shotEstimate: low)

        XCTAssertGreaterThan(lowResult.shot.distanceDispersionYards, highResult.shot.distanceDispersionYards)
        XCTAssertGreaterThan(lowResult.shot.lateralDispersionYards, highResult.shot.lateralDispersionYards)
        XCTAssertLessThan(lowResult.shot.carryYards, highResult.shot.carryYards)
    }

    func testArcadeShotEngineAppliesLandingZoneBias() {
        let hole = HoleState(definition: ArcadeCourseLibrary.starterCourse.holes[1])
        let left = makeShotEstimate(
            ballSpeed: SpeedRange(min: 95, max: 100),
            landingZone: "left",
            confidence: "medium"
        )
        let right = makeShotEstimate(
            ballSpeed: SpeedRange(min: 95, max: 100),
            landingZone: "right",
            confidence: "medium"
        )

        // Same swingID so the per-swing jitter is identical and the symmetry holds.
        let leftResult = ArcadeShotEngine().resolveShot(hole: hole, club: "7", swingID: "shared", shotEstimate: left)
        let rightResult = ArcadeShotEngine().resolveShot(hole: hole, club: "7", swingID: "shared", shotEstimate: right)

        XCTAssertLessThan(leftResult.shot.endPosition.x, 0)
        XCTAssertGreaterThan(rightResult.shot.endPosition.x, 0)
        XCTAssertEqual(abs(leftResult.shot.lateralMissYards), abs(rightResult.shot.lateralMissYards), accuracy: 0.001)
    }

    func testArcadeShotEngineLateralJitterDiffersBetweenSwingsButIsDeterministic() {
        let hole = HoleState(definition: ArcadeCourseLibrary.starterCourse.holes[1])
        let estimate = makeShotEstimate(
            ballSpeed: SpeedRange(min: 95, max: 100),
            landingZone: "left",
        ...
        let firstA = ArcadeShotEngine().resolveShot(hole: hole, club: "7", swingID: "swing-A", shotEstimate: estimate)
        let firstB = ArcadeShotEngine().resolveShot(hole: hole, club: "7", swingID: "swing-B", shotEstimate: estimate)
        let repeatA = ArcadeShotEngine().resolveShot(hole: hole, club: "7", swingID: "swing-A", shotEstimate: estimate)

        XCTAssertEqual(firstA.shot.lateralMissYards, repeatA.shot.lateralMissYards, accuracy: 0.001,
                       "Same swing ID must produce identical jitter.")
        XCTAssertNotEqual(firstA.shot.lateralMissYards, firstB.shot.lateralMissYards,
                          "Different swing IDs should produce different magnitudes.")
        XCTAssertLessThan(firstA.shot.lateralMissYards, 0)
        XCTAssertLessThan(firstB.shot.lateralMissYards, 0)
    }

    func testArcadeShotEngineCenterShotsDriftOnlyOnLowConfidence() {
        let hole = HoleState(definition: ArcadeCourseLibrary.starterCourse.holes[1])
        let highCenter = makeShotEstimate(
            ballSpeed: SpeedRange(min: 95, max: 100),
            landingZone: "center",
            confidence: "high"
        )
        let lowCenter = makeShotEstimate(
            ballSpeed: SpeedRange(min: 95, max: 100),
            landingZone: "center",
            confidence: "low"
        )

        let highResult = ArcadeShotEngine().resolveShot(hole: hole, club: "7", swingID: "high-center", shotEstimate: highCenter)
        let lowResult = ArcadeShotEngine().resolveShot(hole: hole, club: "7", swingID: "low-center", shotEstimate: lowCenter)

        XCTAssertEqual(highResult.shot.lateralMissYards, 0, accuracy: 0.001,
                       "High-confidence center shots stay on the centerline.")
        XCTAssertNotEqual(lowResult.shot.lateralMissYards, 0,
                          "Low-confidence center shots must drift a little.")
    }

    func testRoundRecordUndoLastShotRevertsBallAndStrokes() {
        var round = RoundRecord.starter(
            id: "round-undo",
            startedAt: Date(timeIntervalSince1970: 1_700_000_000)
        )
        let hole = round.holes[0]
        let estimate = makeShotEstimate(
            ballSpeed: SpeedRange(min: 78, max: 82),
            landingZone: "center",
            confidence: "high"
        )
        ...
        XCTAssertGreaterThan(round.totalStrokes, 0)

        let didUndo = round.undoLastShot()

        XCTAssertTrue(didUndo)
        XCTAssertEqual(round.totalStrokes, 0)
        XCTAssertEqual(round.holes[0].strokes, 0)
        XCTAssertEqual(round.holes[0].shots.count, 0)
        XCTAssertEqual(round.holes[0].ballPosition, ArcadePoint(x: 0, y: 0))
        XCTAssertEqual(round.holes[0].lie, .tee)
        XCTAssertFalse(round.holes[0].completed)
        XCTAssertEqual(round.status, .inProgress)
    }

    func testRoundRecordRestartHolesClearsAllStrokes() {
        var round = RoundRecord.starter()
        let estimate = makeShotEstimate(
            ballSpeed: SpeedRange(min: 78, max: 82),
            landingZone: "center",
        ...
        XCTAssertEqual(round.totalStrokes, 0)
        XCTAssertEqual(round.status, .inProgress)
        XCTAssertTrue(round.holes.allSatisfy { $0.shots.isEmpty })
        XCTAssertTrue(round.holes.allSatisfy { !$0.completed })
        XCTAssertEqual(round.holes.count, ArcadeCourseLibrary.starterCourse.holes.count)
    }

    func testRoundRecordAbandonMarksRoundComplete() {
        var round = RoundRecord.starter()
        let timestamp = Date(timeIntervalSince1970: 1_700_500_000)

        round.abandon(at: timestamp)

        XCTAssertEqual(round.status, .complete)
        XCTAssertEqual(round.endedAt, timestamp)
    }

    func testArcadeShotEngineFoamBallIgnoresBallSpeedForCarry() {
        let hole = HoleState(definition: ArcadeCourseLibrary.starterCourse.holes[1])
        let estimate = makeShotEstimate(
            clubSpeed: SpeedRange(min: 78, max: 82),
            ballSpeed: SpeedRange(min: 170, max: 180),
            landingZone: "right",
            confidence: "high"
        )

        let realResult = ArcadeShotEngine().resolveShot(
            hole: hole,
            club: "7",
            ballMode: .realBall,
            swingID: "real",
        ...
        let foamResult = ArcadeShotEngine().resolveShot(
            hole: hole,
            club: "7",
            ballMode: .foamBall,
            swingID: "foam",
        ...
        XCTAssertEqual(foamResult.shot.ballMode, .foamBall)
        XCTAssertLessThan(foamResult.shot.carryYards, realResult.shot.carryYards)
        XCTAssertGreaterThan(foamResult.shot.endPosition.x, 0)
        XCTAssertGreaterThan(foamResult.shot.distanceDispersionYards, realResult.shot.distanceDispersionYards)
        XCTAssertEqual(foamResult.shot.confidence, "medium")
    }

    func testArcadeShotEngineNoBallIgnoresLandingZoneAndForcesLowConfidence() {
        let hole = HoleState(definition: ArcadeCourseLibrary.starterCourse.holes[1])
        let estimate = makeShotEstimate(
            clubSpeed: SpeedRange(min: 78, max: 82),
            ballSpeed: SpeedRange(min: 170, max: 180),
        ...
        XCTAssertEqual(result.shot.ballMode, .noBall)
        XCTAssertEqual(result.shot.confidence, "low")
        XCTAssertEqual(result.shot.lateralMissYards, 0, accuracy: 0.001)
        XCTAssertEqual(result.shot.endPosition.x, 0, accuracy: 0.001)
    }

    func testArcadeShotEngineAutoFinishesOnGreen() {
        let hole = HoleState(
            id: "short-hole",
            holeNumber: 1,
            par: 3,
            lengthYards: 102,
            fairwayWidthYards: 34,
            greenCenter: ArcadePoint(x: 0, y: 102),
            greenRadiusYards: 18,
            outOfBoundsWidthYards: 90
        )
        let estimate = makeShotEstimate(
            ballSpeed: SpeedRange(min: 78, max: 82),
            landingZone: "center",
        ...
        XCTAssertEqual(result.shot.lie, .green)
        XCTAssertGreaterThan(result.shot.autoPutts, 0)
        XCTAssertTrue(result.updatedHole.completed)
        XCTAssertEqual(result.updatedHole.strokes, 1 + result.shot.autoPutts)
    }

    func testRoundShotDecodesLegacyJSONWithoutBallMode() throws {
        let json = """
        {
          "id": "legacy-shot",
          ...
          "holeNumber": 1,
          "club": "7",
          "swingID": "swing-legacy",
          "startPosition": { "x": 0, "y": 0 },
          "endPosition": { "x": 0, "y": 142 },
          "carryYards": 142,
          "lateralMissYards": 0,
          "distanceDispersionYards": 16,
        ...
        let shot = try decoder.decode(RoundShot.self, from: Data(json.utf8))

        XCTAssertEqual(shot.ballMode, .realBall)
    }

    func testRoundRepositoryMissingFileAndPersistence() throws {
        let root = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: root) }

        let repository = RoundRepository(rootDirectory: root)
        ...
        try repository.save([round])

        XCTAssertEqual(try repository.load(), [round])
    }
    ...
    private func makeShotEstimate(
        clubSpeed: SpeedRange? = nil,
        ballSpeed: SpeedRange? = nil,
        landingZone: String,
        confidence: String
    ) -> ShotEstimate {
        ShotEstimate(
            ...
            evidence: ShotEstimateEvidence(
                ...
                detectedBallFlightFrames: ballSpeed == nil ? 0 : 4
            ),
            limitations: []
        )
    }

    private func makeFixtureAnalysis(
        cameraAngle: String = "down_the_line",
        includeShotEstimate: Bool = true
    ) throws -> SwingAnalysis {
        let shotEstimateJSON = includeShotEstimate ? """
          "shotEstimate": {
            "clubSpeedMphRange": { "min": 76.4, "max": 105.8, "unit": "mph" },
            "ballSpeedMphRange": { "min": 102.1, "max": 171.3, "unit": "mph" },
            "landingZone": "right",
            "confidence": "medium",
            "evidence": {
              "sourceFps": 120,
              "calibrationSource": "selected_club_address_length",
              "detectedBallFlightFrames": 4
            },
            "limitations": []
          },
        """ : ""
